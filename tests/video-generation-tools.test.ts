...
describe('video generation tool', () => {
  it('auto-approves bounded loopback generation without public-network permission', () => {
    const tool = createVideoGenerationTools({ probeVideo: mockVideoProbe,
  ...
  it('generates a text-to-video MP4 through the Wan media service', async () => {
    ...
    const tool = createVideoGenerationTools({ probeVideo: mockVideoProbe, env: { DACAI_VIDEO_BACKEND: 'dacais-media' }, fetch: fetchMock as typeof fetch })[0];
    ...
    expect(result).toMatchObject({ path: 'output/ocean.mp4', format: 'mp4', backend: 'dacais-media', frames: 480, durationSeconds: 30 });
  ...
  it('animates a workspace image and fails closed when the backend is disabled', async () => {
    ...
    const disabled = createVideoGenerationTools({ probeVideo: mockVideoProbe, env: {}, fetch: fetchMock as typeof fetch })[0];
  ...
  it('requires explicit authenticated HTTPS for production backends and rejects unsafe paths/data/overwrites', async () => {
    const root = await workspace();
    const remote = createVideoGenerationTools({ probeVideo: mockVideoProbe,
    ...
    const production = createVideoGenerationTools({ probeVideo: mockVideoProbe,
      env: {
        DACAI_VIDEO_BACKEND: 'dacais-media', DACAI_MEDIA_TRANSPORT: 'https',
    ...
    const invalid = createVideoGenerationTools({ probeVideo: mockVideoProbe,
    ...
    const valid = createVideoGenerationTools({ probeVideo: mockVideoProbe,
      env: { DACAI_VIDEO_BACKEND: 'dacais-media' },
