# Capability 99: Voice-driven coding-agent interface
...
Provide DacaiLocalAgent with a reusable workflow for **Voice-driven coding-agent interface**.
...
- configured speech input/output provider
...
- Voice input is another user interface to the same permissioned task graph.
- Speech must not bypass confirmations required for typed requests.
